...
# def prep(txt, width=42): # 42 حرف هو المقاس المثالي لعرض عمودك
#     if not txt: return "-"
    
#     # 1. نقسم النص بناءً على أسطر الطالب الأصلية (إذا كان ضاغط إنتر)
#     lines = str(txt).split('\n')
#     final_lines = []
    
#     for line in lines:
#         # 2. السحر هنا: نقص السطر الطويل لأسطر قصيرة قبل لا نقلبه!
...
#             # 3. نربط الحروف
#             reshaped = reshaper.reshape(w_line)
#             # 4. نعكس كل سطر قصير لحاله عشان يحافظ على مكانه من فوق لتحت
#             bidi_line = get_display(reshaped, base_dir='R')
#             final_lines.append(bidi_line)
            
#     # 5. ندمج الأسطر بعلامة النزول لسطر جديد في ReportLab
...
from reportlab.lib.units import mm, cm
from reportlab.lib.enums import TA_RIGHT, TA_LEFT, TA_CENTER
from reportlab.lib.utils import simpleSplit  # 👈 استيراد أداة القص الذكي بالملمتر
...
def prep(txt, max_width_mm=190):  # القيمة الافتراضية 190 ملم وهي العرض الكلي المتاح للصفحة
    if not txt: 
        return "-"
    
    # 1. ربط الحروف العربية مع الحفاظ على المصطلحات الإنجليزية والأرقام
    reshaped = reshaper.reshape(str(txt))
    
    # 2. تقسيم النص إذا كان يحتوي على أسطر يدوية (\n)
    paragraphs = reshaped.split('\n')
    
    final_lines = []
    for p in paragraphs:
        if not p.strip():
            continue
        # 3. السحر هنا: قص السطر فيزيائياً بالملمتر بناءً على نوع الخط وحجمه قبل عملية القلب
        # هذا يمنع ReportLab من إعادة قص النص داخلياً وتخريب الترتيب العربي/الإنجليزي
        lines = simpleSplit(p, "Arial", 9, max_width_mm * mm)
        
        for line in lines:
            # 4. قلب كل سطر قصير على حدة مع تحديد الاتجاه الأساسي لليمين (R) لحماية النص المختلط
            bidi_line = get_display(line, base_dir='R')
            final_lines.append(bidi_line)
            
    # 5. دمج الأسطر بعلامة النزول لسطر جديد المعتمدة في ReportLab
    return "<br/>".join(final_lines) if final_lines else "-"
...
def download_exam_report(
    ...
    record = db.execute(
        text("""
            ...
            JOIN teacher_courses tc ON f.tc_id = tc.tc_id
            JOIN courses c ON tc.course_id = c.course_id
            LEFT JOIN level l ON c.level_id = l.level_id
            WHERE ans.student_id = :sid AND e.exam_id = :eid
        """),
        {"sid": student_id, "eid": exam_id},
    ).mappings().first()
    ...
    st = {
        ...
        "subtitle": s("subtitle", fontName="Arial", fontSize=12, leading=18, textColor=GRAY_TEXT, alignment=TA_CENTER, spaceAfter=15),
        "table_header": s("table_header", fontName="Arial-Bold", fontSize=10, leading=16, textColor=colors.white, alignment=TA_CENTER),
        "table_cell": s("table_cell", fontName="Arial", fontSize=9, leading=14, textColor=TEXT_COLOR, alignment=TA_CENTER),
        "info_label": s("info_label", fontName="Arial", fontSize=10, leading=16, textColor=GRAY_TEXT, alignment=align_main),
        "info_value": s("info_value", fontName="Arial", fontSize=11, leading=16, textColor=TEXT_COLOR, alignment=align_main),
        "section_title": s("section_title", fontName="Arial-Bold", fontSize=12, leading=18, textColor=TEXT_COLOR, alignment=align_main),
        "badge_text": s("badge_text", fontName="Arial-Bold", fontSize=11, leading=16, textColor=colors.white, alignment=TA_CENTER),
    ...
    # Header Titles
    elems.append(Paragraph(prep(tr("نظام التصحيح الذكي", "Smart Grading System"), 190), st["title"]))
    elems.append(Paragraph(prep(tr("تقرير نتيجة اختبار تفصيلي", "Detailed Exam Report"), 190), st["subtitle"]))
    elems.append(HRFlowable(width="100%", thickness=2, color=TEAL, spaceAfter=10))
    ...
    if is_ar:
        info_data = [
            [Paragraph(prep(exam_lbl, 90), st["info_value"]), Paragraph(prep(stu_lbl, 90), st["info_value"])],
            [Paragraph(prep(date_lbl), st["info_label"]), Paragraph(prep(lvl_lbl), st["info_label"])],
        ]
    else:
        info_data = [
            [Paragraph(prep(stu_lbl, 90), st["info_value"]), Paragraph(prep(exam_lbl, 90), st["info_value"])],
            [Paragraph(prep(lvl_lbl), st["info_label"]), Paragraph(prep(date_lbl), st["info_label"])],
        ]
    ...
    if is_ar:
        badge_data = [[Paragraph(prep(score_label, 60), st["badge_text"]), Paragraph(prep(sec_title, 130), st["section_title"])]]
        badge_bg = [("BACKGROUND", (0, 0), (0, 0), TEAL), ("ALIGN", (1, 0), (1, 0), "RIGHT")]
    else:
        badge_data = [[Paragraph(prep(sec_title, 130), st["section_title"]), Paragraph(prep(score_label), st["badge_text"])]]
        badge_bg = [("BACKGROUND", (1, 0), (1, 0), TEAL), ("ALIGN", (0, 0), (0, 0), "LEFT")]
    ...
    # ── 3. Questions Table Structure ──
    usable_w = PAGE_W - 2 * MARGIN
    col_num, col_score, col_q = 14 * mm, 20 * mm, 50 * mm
    col_ans = (usable_w - col_num - col_score - col_q) / 2 # يعادل 53 ملم تقريباً لكل عمود إجابة
    ...
    rows = [[Paragraph(prep(h, 50), st["table_header"]) for h in headers]]
    ...
    for i, q in enumerate(questions):
        ...
        # 🌟 هنا تكمن القوة الاحترافية: نمرر العرض الفعلي الدقيق لكل عمود بالملمتر (ناقص 5 ملم أمان للهوامش)
        # هذا يضمن حماية الكلمات الإنجليزية والعربية المختلطة من التقاطع والانعكاس التلقائي
        if is_ar:
            row_cells = [
                Paragraph(prep(mark_str, 15), st["table_cell"]),
                Paragraph(prep(m_ans, 48), st["table_cell"]),
                Paragraph(prep(s_ans, 48), st["table_cell"]),
                Paragraph(prep(q["question_text"], 45), st["table_cell"]),
                Paragraph(prep(str(q["question_order"]), 10), st["table_cell"])
            ]
        else:
            row_cells = [
                Paragraph(prep(str(q["question_order"]), 10), st["table_cell"]),
