...
class _AdminSettingsScreenState extends State<AdminSettingsScreen>
                      ...
                      child: TabBarView(
                        ...
                        children: [
                          SingleChildScrollView(
                            ...
                            child: isMobile
                                ? _buildAccountMobileLayout(provider)
                                : _buildAccountWebLayout(provider),
                          ),
                          SingleChildScrollView(
                            padding: EdgeInsets.all(isMobile ? 15 : 40),
                            child: _buildAcademicLayout(isMobile, provider),
                          ),
                        ],
                      ),
                    ),
  ...
  Widget _buildAccountWebLayout(AdminSettingsProvider provider) => Row(
    ...
    children: [
      Expanded(
        ...
        child: Column(
          children: [
            _buildProfileCard(false, provider),
      ...
      Expanded(
        ...
        child: Column(
          children: [
            _buildDisplayPreferencesCard(provider),
            const SizedBox(height: 25),
            _buildDangerZoneCard(provider),
          ],
        ),
      ),
    ],
  ...
  Widget _buildAccountMobileLayout(AdminSettingsProvider provider) => Column(
    children: [
      _buildProfileCard(true, provider),
      const SizedBox(height: 20),
      _buildSecurityCard(provider),
      const SizedBox(height: 20),
      _buildDisplayPreferencesCard(provider),
                    ...
                    onPressed: () {
                      if (_nameController.text.trim().isNotEmpty &&
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(S.of(context).err_missing_data),
        ...
        children: [
          ...
          _buildActionRow(
            ...
            S
                .of(context)
                .settingsLastChange(
                  provider.lastPasswordChange.isEmpty
                      ? S.of(context).not_specified
                      : provider.lastPasswordChange,
                ),
            Icons.lock_outline,
            actionLabel: S.of(context).settingsChangePassword,
            onTap: () => _showPasswordDialog(provider),
          ),
        ],
      ),
    );
            ...
            children: [
              ...
              _buildDialogInputField(
                ...
                onChanged: (val) => setDialogState(
                  () => provider.checkPasswordStrength(val, context),
                ),
              ),
              const SizedBox(height: 15),
              _buildDialogInputField(
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (context, setState) {
            bool isLoading =
                provider.isSendingForgotOtp ||
                provider.isVerifyingForgotOtp ||
                provider.isResettingForgotPass;
                ...
                children: [
                  ...
                  if (step == 3) ...[
                    ...
                    _buildDialogInputField(
                      ...
                      isObscured: obsConfirm,
                      icon: Icons.lock_outline,
                      onToggle: () => setState(() => obsConfirm = !obsConfirm),
                    ),
                  ],
                ],
              ),
              actions: [
                ...
                ElevatedButton(
                  onPressed: isLoading
                      ...
                      : () async {
                          if (step == 1) {
                            bool sent = await provider.sendForgotPasswordOtp(
                              context,
                            );
                            ...
                            bool success = await provider.resetPasswordWithOtp(
                              _otpController.text,
                              _newPassController.text,
                              context,
                            );
  ...
  Widget _buildLanguageSelector(
    ...
    child: DropdownButtonHideUnderline(
      child: DropdownButton<String>(
        ...
        isExpanded: true,
        items: [
          DropdownMenuItem(value: 'ar', child: Text(S.of(context).arabic_lang)),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                S.of(context).academic_semesters_title,
                style: TextStyle(
              ...
              ElevatedButton(
                onPressed: () => _showAddSemesterDialog(provider),
                style: ElevatedButton.styleFrom(
                  backgroundColor: AppColors.primaryTeal(context),
                  padding: const EdgeInsets.symmetric(
                    horizontal: 20,
    ...
    child: Padding(
      ...
      child: Column(
        children: [
          ...
          Text(
            S.of(context).no_semesters_added,
            style: const TextStyle(
              color: Colors.grey,
              fontSize: 16,
              fontWeight: FontWeight.bold,
        ...
        rows: List.generate(provider.semestersList.length, (index) {
          ...
          return DataRow(
            cells: [
              ...
              DataCell(
                Text(semester['start_date'] ?? S.of(context).not_specified),
              ),
              DataCell(
                Text(semester['end_date'] ?? S.of(context).not_specified),
              ),
              DataCell(
                Switch(
                  value:
                  ...
                  activeColor: AppColors.primaryTeal(context),
                  onChanged: (val) =>
                      provider.toggleCurrentSemester(index, context),
                ),
              ),
              DataCell(
                Row(
                  children: [
                    IconButton(
                      ...
                      onPressed: () =>
                          _showEditSemesterDialog(provider, semester),
                    ),
                    IconButton(
                      icon: const Icon(
                      ...
                      onPressed: () =>
                          _showConfirmDeleteDialog(provider, semester),
                    ),
                  ],
                ),
              ),
      ...
      content: StatefulBuilder(
        builder: (context, setDialogState) {
          return Column(
            ...
            children: [
              _buildDialogInputField(
                label: S.of(context).semester_name_hint,
                controller: _semesterNameController,
                icon: Icons.bookmark_border,
              ),
              const SizedBox(height: 15),
              _buildDialogInputField(
                label: S.of(context).academic_year_hint,
  ...
  void _showEditSemesterDialog(
    ...
    _semesterNameController.text = semester['semester_name'] ?? "";
    _academicYearController.text = semester['academic_year'] ?? "";
    _startDate = semester['start_date'] != null
        ? DateTime.tryParse(semester['start_date'])
        : null;
    _endDate = semester['end_date'] != null
        ? DateTime.tryParse(semester['end_date'])
        : null;
    _showCustomDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
              provider.deleteSemester(semester['semester_id'], context);
            },
            child: Text(
  ...
  Widget _buildDialogInputField({
    ...
    return Column(
      ...
      children: [
        ...
        if (showStrength &&
            provider != null &&
            provider.passwordStrengthText.isNotEmpty) ...[
          const SizedBox(height: 12),
          Text(
            S.of(context).settingsStrengthLabel(provider.passwordStrengthText),
            style: TextStyle(
          ...
          LinearProgressIndicator(
            value: provider.passwordStrengthValue,
        ...
        Row(
          children: [
            ...
            Column(
              ...
              children: [
                Text(
                  S.of(context).admin_settings_title,
                  style: TextStyle(
                    fontSize: 22,
                    fontWeight: FontWeight.bold,
                    color: AppColors.textPrimary(context),
                ...
                Text(
                  S.of(context).admin_settings_subtitle,
                  style: TextStyle(
                    color: AppColors.textSecondary(context),
                    fontSize: 14,
                  ),
